...
func (c *containerLXC) initLXC() error {
	...
	// Setup devlxd
	err = lxcSetConfigItem(cc, "lxc.mount.entry", fmt.Sprintf("%s dev/xlxd none bind,create=dir 0 0", shared.VarPath("devlxd")))
	if err != nil {
		return err
	}
	...
	// Setup shmounts
	err = lxcSetConfigItem(cc, "lxc.mount.entry", fmt.Sprintf("%s dev/.xlxd-mounts none bind,create=dir 0 0", shared.VarPath("shmounts", c.Name())))
	if err != nil {
		return err
	}
...
func (c *containerLXC) insertMount(source, target, fstype string, flags int) error {
	...
	mntsrc := filepath.Join("/dev/.xlxd-mounts", filepath.Base(tmpMount))
